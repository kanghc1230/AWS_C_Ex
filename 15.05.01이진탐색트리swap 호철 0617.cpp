...
int main(void)
{
	...
	while (1) {
		...
		if (command == 1) {
			...
			swap_result = swaptree(root);
			treeprint(swap_result);
			printf("\n");
		}
		else {
			printf("\nCommand error\n");
...
struct tnode* swaptree(struct tnode* original) {
	struct tnode* temp;
	if (original) {			//original이 NULL이 아니면 생성 
		temp = (tnode*)malloc(sizeof(tnode));

		temp->left = swaptree(original->right);
		temp->right = swaptree(original->left);
		temp->item = original->item;

		return temp; //생성하고 되돌려줌
	}
	return NULL;
}