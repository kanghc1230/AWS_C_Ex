...
void read_line(char*, int);//유저로부터 중위수식받는함수
...
void push(double);//계산스택함수2 push
double pop(void); //계산삭제함수3 pop
int sp = 0;// 스택위치
...
void print_token(precedence item) {	//원래 출력용이지만 (바뀐값 전달같이함) de
	expr2[expr2_num++] = ' ';	//연산자 구분 공백
	switch (item) {
	case 2:
		expr2[expr2_num++] = '+'; break;
	case 3:
		expr2[expr2_num++] = '-'; break;
	case 4:
		expr2[expr2_num++] = '*'; break;
...
void postfix(void)
{
	...
	{	//초기값token = precedence;		token == eos;면끝. token = precedence;다음계산
		...
		if (token == operand) {   // operand피연산자(숫자)가 나오면 바로출력
			expr2[expr2_num++] = symbol;		//첫값넣기
			if (symbol2 != 0) {
				expr2[expr2_num++] = symbol2;	//둘째값있으면넣기
			}
			if (symbol3 != 0) {
				expr2[expr2_num++] = symbol3;	//셋째값있으면넣기
			}
			expr2[expr2_num++] = ' ';	//끝나면 나누기 위한 공백 추가
		}
...
void read_line(char* input, int len)
{
	...
	int i = 0;
	printf("입력식 : ");
	while (((blankline[i++] = getchar()) != '\n') && i < len);	//엔터치거나 최대100까지 받아서
	blankline[--i] = '\0';

	int k_i = 0;
	for (int k = 0; k <= i; k++) {			//blankchar제거
		if (blankline[k] != ' ')
			input[k_i++] = blankline[k];
	}
	if (i >= len) {							//수식 100초과
